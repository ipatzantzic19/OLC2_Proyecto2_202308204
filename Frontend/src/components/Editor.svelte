<script lang="ts">
  ...
  function newFile()  { if (!confirm('¿Descartar cambios?')) return; editorCode.set('func main() {\n    \n}'); clearAll(); }
  function loadFile() {
    const i = document.createElement('input'); i.type = 'file'; i.accept = '.go,.golampi';
    i.onchange = (e: any) => { const f = e.target.files?.[0]; if (!f) return; const r = new FileReader(); r.onload = (ev: any) => { editorCode.set(ev.target.result); fileName = f.name; }; r.readAsText(f); };
    i.click();
  }
  ...
  <header class="top-bar">
    <div class="bar-left">
      ...
      <button class="btn" on:click={newFile}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><path d="M14 2v6h6"/><line x1="12" y1="11" x2="12" y2="17"/><line x1="9" y1="14" x2="15" y2="14"/></svg>
        Nuevo
      </button>
    ...
    <section class="editor-pane">
      <div class="pane-tabs">
        <div class="pane-tab active">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="#4A9EFF"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/></svg>
          {fileName}
        </div>
      </div>
  ...
  <footer class="bottom-bar">
    <div class="bar-left">
      ...
      <button class="btn btn-report" on:click={showErrors}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><line x1="12" y1="8" x2="12" y2="13"/><circle cx="12" cy="17" r="1" fill="currentColor"/></svg>
        Errores
      ...
      <button class="btn btn-report" on:click={showSymbols}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="16" rx="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="8" y1="4" x2="8" y2="20"/></svg>
        Símbolos
...
.top-bar { display: flex; justify-content: space-between; align-items: center; height: 50px; padding: 0 14px; gap: 8px; background: #2D2D2D; border-bottom: 1px solid #3E3E3E; flex-shrink: 0; }
.bar-left { display: flex; align-items: center; gap: 6px; }
.bar-right { display: flex; align-items: center; gap: 8px; }
.logo { display: flex; align-items: center; gap: 7px; padding-right: 10px; }
.logo-name { font-size: 15px; font-weight: 700; color: #E0E0E0; }
.logo-ide { color: #4A9EFF; }
.logo-badge { padding: 2px 6px; background: rgba(197,134,192,.2); border: 1px solid rgba(197,134,192,.4); border-radius: 4px; font-size: 10px; font-weight: 700; color: #C586C0; }
.sep { width: 1px; height: 22px; background: #3E3E3E; margin: 0 4px; }
.btn { display: flex; align-items: center; gap: 5px; padding: 5px 11px; background: transparent; border: 1px solid #3E3E3E; border-radius: 4px; color: #D4D4D4; font-size: 12px; cursor: pointer; transition: all .15s; white-space: nowrap; }
.btn:hover:not(:disabled) { background: #3E3E3E; border-color: #555; }
.btn:disabled { opacity: .4; cursor: not-allowed; }
.btn-compile { background: #4A9EFF; border-color: #4A9EFF; color: #fff; font-weight: 700; font-size: 13px; padding: 6px 14px; }
...
.pane-tabs { display: flex; background: #252526; border-bottom: 1px solid #3E3E3E; flex-shrink: 0; }
